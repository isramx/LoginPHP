...
if (isset($_POST['accion'])) {
    ...
    if (!empty($_FILES['imagen']['name'])) {
        ...
        $imagen_nombre = $codigo . '_' . time() . '.' . $ext;
        move_uploaded_file($_FILES['imagen']['tmp_name'], $carpeta_imagenes . $imagen_nombre);
    }

    if ($accion === 'agregar' && $codigo != '') {
        if (isset($inventario[$codigo])) {
            $inventario[$codigo]['cantidad'] += $cantidad;
            $inventario[$codigo]['fecha'] = $fecha;
            if ($imagen_nombre) $inventario[$codigo]['imagen'] = $imagen_nombre;
        } else {
            $inventario[$codigo] = [
                'codigo' => $codigo,
                'nombre' => $nombre,
    ...
    if ($accion === 'restar' && isset($inventario[$codigo])) {
        $inventario[$codigo]['cantidad'] -= $cantidad;
        if ($inventario[$codigo]['cantidad'] < 0) $inventario[$codigo]['cantidad'] = 0;
        $inventario[$codigo]['fecha'] = $fecha;
    }

    if ($accion === 'editar' && isset($inventario[$codigo])) {
    ...
    if ($accion === 'eliminar' && isset($inventario[$codigo])) {
        // Eliminar imagen asociada si existe
        if (!empty($inventario[$codigo]['imagen'])) {
            $ruta = $carpeta_imagenes . $inventario[$codigo]['imagen'];
    ...
    echo '<div class="alert alert-success">Inventario actualizado.</div>';
}
?>
...
<form method="POST" enctype="multipart/form-data">
    <div class="row">
        <div class="col-sm-3">
            <input type="text" name="codigo" placeholder="Código" class="form-control" required>
        </div>
        <div class="col-sm-3">
        ...
        <div class="col-sm-2">
            <button name="accion" value="agregar" class="btn btn-success">➕ Agregar</button>
            <button name="accion" value="restar" class="btn btn-warning">➖ Restar</button>
            <button name="accion" value="editar" class="btn btn-info">✏️ Editar</button>
            <button name="accion" value="eliminar" class="btn btn-danger" onclick="return confirm('¿Eliminar este código?')">🗑 Eliminar</button>
        </div>
    </div>
</form>
